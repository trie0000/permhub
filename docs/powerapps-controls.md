...
高さの自動調整（`AutoHeight`）を一時的に ON にすると、`Height` に**内容の高さ**が入る。
パディング 0 で測ると `Size: =13` → `Height: =18`。これが 1 行の高さ。

| Size | 10 | 11 | 12 | 13 | 15 | 16 |
|---|---|---|---|---|---|---|
| 行高 | 14 | 15 | 17 | 18 | 21 | 22 |
...
> **なぜ既定の 5px だと「だいたい中央」に見えるのか。**
> `Height: =34` / `Size: =13` なら中央にするための `PaddingTop` は (34-18)/2 = 8。
> 既定の 5 とは 3px しか違わないので気づきにくい。**0 にすると 8px ずれて一気に上詰めに見える。**
> ここを取り違えて「`Wrap: =false` が原因」と誤診したことがある。`Wrap` は縦位置に関係しない。
