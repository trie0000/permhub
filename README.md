...
| **管理者メニュー（Teams グループのメンバーだけに表示）** | **完了・実機検証済み** |
| **管理者の申請一覧（未対応が分かる／未完了のみ絞り込み）** | **完了・実機検証済み** |
| **申請詳細のサブ切り替え（組織マスタ／ユーザマスタ）** | **完了・実機検証済み** |
| **明細ごとの 作業待ち／完了／差し戻し と一括変更** | **完了・実機検証済み** |
| **「完了」でマスタへ反映（差し戻しは何も反映しない）** | **完了・実機検証済み** |
...
- `ScrHome` — 3 タブ + 管理タブ。組織区分1 セレクタは組織マスタ／ユーザマスタ タブの中
  （選んだ値は 3 タブすべてに効く。申請履歴・管理タブにはセレクタを出さない）
- `ScrUser` — 利用者の基本情報・所属タブ・権限マスタ／ディテール
- `ScrReq` — 申請ヘッダと明細。管理者はここで明細ごとに状態を変えて反映する
- **管理タブは `PRM_Config.AdminGroupId` の Teams グループのメンバーにだけ出る**
...
明細が 1 つでも残っていれば「作業待ち」、完了と差し戻しが混ざれば「一部完了」。
